...
A running record of issues caught during review passes — what went wrong, how the issue surfaced, and how the fix was structured. This file is the "what didn't work the first time" record; `ai-audit-log.md` complements it with the AI-specific subset suitable for EXPLAINER Q5.
...
- **How caught:** Review probe hit the endpoint with a nonexistent merchant id; expected 404, observed 500 with `IdempotencyKey.DoesNotExist` in the trace.
...
- **How caught:** Live shell probe ran `str(IdempotencyKey)` after the migration applied.
...
- **How caught:** Review pass grepped for `django_celery_beat` references after observing only the file-based scheduler running at boot.
...
- **How caught:** Review pass noticed the file in the working tree after Phase 4 boot.
...
- **How caught:** Review pass ran two back-to-back `retry_payout` calls and observed `retry_count` jumping non-deterministically.
...
- **Symptom:** Phase 7 was called "complete" after writing README + EXPLAINER, but the rubric requires a *live deployment URL* and there were no deploy artefacts in the repo: no `Procfile`, no `runtime.txt`, no `nixpacks.toml`, no production security headers in `settings.py`, no production frontend serving path. The README itself admitted this with `"Phase 8 deploy notes (TBD)"`.
- **How caught:** Review pass searched the repo for deploy artefacts (`find . -name "Procfile*" -o -name "railway.*" -o -name "Dockerfile*"` returned empty) and grepped `settings.py` for `SECURE_*` markers (also empty), then noted the README's own self-admitted gap.
...
When a review surfaces something, append a section using this template:
